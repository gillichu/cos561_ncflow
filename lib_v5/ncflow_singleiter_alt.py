...
def r1_lp(G, paths_dict, agg_commodities_dict, inter_edges):
        ...
        for k, (s_k, t_k, d_k) in meta_commodity_list:
            ...
            path_ids = []
            path = paths[0]
            print(path, list(path_to_edge_list(path)))
        ...
        print('--> R1 #total paths=', sum(num_paths),
...
def solve(G, tm, iter_id, num_clusters,G_agg, agg_edge_dict, agg_to_orig_nodes, orig_to_agg_node, G_clusters_dict, agg_commodities_dict,clusters_commodities_dict, hash_for_clusterid, meta_to_virt_dict, virt_to_meta_dict, commodity_list):
    ...
    def compute_flow_per_inter_commod(meta_commodity_dict, inter_sol_dict, kirchoff_flow_per_commod, waterfall_memoized):
        ...
        for meta_commod_key, orig_commod_list_in_k_meta in meta_commodity_dict.items():
            ...
            print('meta_commod_key', meta_commod_key, 'r3_meta_flow',r3_meta_flow, sum_of_kirchoff_flows)
            ...
            if abs(sum_of_kirchoff_flows - r3_meta_flow) < EPS:
                ...
                print(sum_of_kirchoff_flows)
                print(adjusted_commods)
                ...
                print('le sum',sum(flow_per_inter_commod[k] for k, _ in adjusted_commods))
    ...
    def waterfall_memoized():
        ...
        def fn(flow_val, k, commods):
            ...
            while len(demand_remaining) > 0:
                ...
                print(demand_remaining[k_smallest])
    ...
    paths = path_meta(G, G_agg, num_clusters, edge_to_bundlecap, 0, k=4) #missing k earlier
    # print('PATH',paths[(0,1)])
    meta_commodity_list = list(agg_commodities_dict.keys())
    # inter_edges = select_inter_edge(G, agg_edge_dict, iter_id)
    ...
    for key, val in r1_sol_dict.items():
        print(key, val, r1_paths[key[0]])
    ...
    for (u_meta, v_meta), reconciliation_sol_dict in reconciliation_solutions_dicts.items():
        ...
        for meta_commod, flow_list in reconciliation_sol_dict.items():
            ...
            # if k_meta in after_recon_flow:
            #     print('already exists', after_recon_flow[k_meta], total_flow)
    ...
    comparison_dict = {}
    for k, (s_k, t_k, d_k) in meta_commodity_list:
        if k not in comparison_dict:
            comparison_dict[k] = []
        comparison_dict[k].append(('meta',d_k))
        

    for k, (s_k, t_k, d_k) in adjusted_meta_commodity_list:
        if k not in comparison_dict:
            print('err in adjust')
        else:
            comparison_dict[k].append(('adj',d_k))
    print()
    for key in comparison_dict.keys():
        print(key, set(comparison_dict[key]))

    print(reconciliation_solutions_dicts)
    # return
                        ...
                        for path_id in path_ids:
                            ...
                            for u, v in path_to_edge_list(r2_all_paths[path_id]):
                                ...
                                if v in virt_to_meta_dict:
                                    ...
                                    print(u,v, path_id, r2_all_paths[path_id])
                                print('key',commod_key[0])
                                # if u==17 and v==66:
                                #     fornow = commod_key[0]
                                #     raise Exception(f"{fornow}: {flow_per_path_per_commod}")
                                sol_dict[commod_key].append(((u, v), flow_per_path_per_commod))

        return sol_dict, r1_sol_dict
